...
# Cria dinamicamente novos Canos
...
while True:
    ...
    if START:  # Se START for True (Verdadeiro) cria um loop no jogo.
        start_game()

    for event in pygame.event.get():
        if event.type == KEYDOWN:
            if event.key == K_ESCAPE:  # Tecla ESC
                pygame.quit()
            if event.key == K_SPACE:  # Tecla Barra de Espaco
                if not START:  # Se START == FALSE (Falso) Prepara o jogo para iniciar.
                    START = True
                    start_game()

                bird.bump()
            # Outro exemplo de iniciar o Jogo
